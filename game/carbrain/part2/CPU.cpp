...
void CPU::tick()
{
    ...
    const Instruction instr = Instruction::decode([&]() -> uint8_t
                                                  { uint8_t operation = bus->read1(registers[REG_IP]++);return operation; },
                                                  bytes_read);
...
spg_register_t CPU::fetchOperand2(std::shared_ptr<CPUInstruction::Operand> operand)
{
    ...
    else if (operand->type == CPUInstructionOperandType::MEM_REGISTER)
    {
        ...
        return htons(bus->read2_be(registers[mem_reg_operand->register_index] + mem_reg_operand->displacement));
    }
    else if (operand->type == CPUInstructionOperandType::MEM_IMMEDIATE)
    {
        ...
        return htons(bus->read2_be(mem_imm_operand->address));
    }
...
uint8_t CPU::fetchOperand1(std::shared_ptr<CPUInstruction::Operand> operand)
{
    using namespace CPUInstruction;
    if (operand->type == CPUInstruction::CPUInstructionOperandType::REGISTER)
    {
...
void CPU::store1(CPUInstruction::Instruction instr, uint8_t source)
{
...
void CPU::store2(CPUInstruction::Instruction instr, spg_register_t source)
{
    ...
    else if (instr.target->type == CPUInstructionOperandType::MEM_REGISTER)
    {
        ...
        std::shared_ptr<MemRegisterOperand> mem_reg_operand = std::dynamic_pointer_cast<MemRegisterOperand>(instr.target);
        bus->write2_be(registers[mem_reg_operand->register_index] + mem_reg_operand->displacement, htons(source));
    }
    else if (instr.target->type == CPUInstructionOperandType::MEM_IMMEDIATE)
    {
        // std::cout << "store2 in mem_immediate" << std::endl;
        std::shared_ptr<AddressOperand> mem_imm_operand = std::dynamic_pointer_cast<AddressOperand>(instr.target);
        bus->write2_be(mem_imm_operand->address, htons(source));
    }
    else if (instr.target->type == CPUInstructionOperandType::IMMEDIATE)
    {
        throw std::runtime_error("Can't store to immediate");
...
void CPU::store2source(CPUInstruction::Instruction instr, spg_register_t source)
{
    // TODO bus->write2(target, source); check target type
    using namespace CPUInstruction;
    if (instr.source->type == CPUInstructionOperandType::REGISTER)
    {
        // std::cout << "store2 in register with value "<< std::to_string(source) << std::endl;
        std::shared_ptr<RegisterOperand> reg_operand = std::dynamic_pointer_cast<RegisterOperand>(instr.source);
        registers[reg_operand->register_index] = source;
    }
    else if (instr.source->type == CPUInstructionOperandType::MEM_REGISTER)
    {
        // std::cout << "store2 in mem_register" << std::endl;
        std::shared_ptr<MemRegisterOperand> mem_reg_operand = std::dynamic_pointer_cast<MemRegisterOperand>(instr.source);
        bus->write2_be(registers[mem_reg_operand->register_index] + mem_reg_operand->displacement, htons(source));
    }
    else if (instr.source->type == CPUInstructionOperandType::MEM_IMMEDIATE)
    {
        // std::cout << "store2 in mem_immediate" << std::endl;
        std::shared_ptr<AddressOperand> mem_imm_operand = std::dynamic_pointer_cast<AddressOperand>(instr.source);
        bus->write2_be(mem_imm_operand->address, htons(source));
    }
    else if (instr.source->type == CPUInstructionOperandType::IMMEDIATE)
    {
        throw std::runtime_error("Can't store to immediate");
    }
    else
...
void CPU::handleInstruction(const CPUInstruction::Instruction &instr)
{
    ...
    case Operation::POP:
    {
        ...
        store2(instr,ntohs(bus->read2_be((registers[REG_SP]))));
        registers[REG_SP] += 2;
        updateflag(ntohs(bus->read2_be(registers[REG_SP])) == 0);
    ...
    {
        spg_register_t source = fetchOperand2(instr.source);
        spg_register_t result = ~source;
        store2source(instr, result);
        updateflag(result == 0);
        // std::cout << "NOT " << std::to_string(source) << std::endl;
        break;
    }
    case Operation::XOR:
    {
    ...
    {
        spg_register_t source = fetchOperand2(instr.source);
        spg_register_t result = source + 1;
        store2source(instr, result);
        updateflag(result == 0);
    ...
    case Operation::CALL:
    {
        ...
        bus->write2_be(registers[REG_SP], ntohs(registers[REG_IP]));
        registers[REG_IP] = source;
        break;
    }
    case Operation::RET:
    {
        registers[REG_IP] = ntohs(bus->read2_be((registers[REG_SP])));
        registers[REG_SP] += 2;
